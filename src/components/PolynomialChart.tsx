...
import { predictPoly } from "@/algorithms/polynomialRegression";
...
export function PolynomialChart({
  train,
  test,
  coefficients,
  showResiduals,
  ...
  const view = useMemo(() => {
    ...
    for (let i = 0; i <= SAMPLES; i++) {
      ...
      curve.push({ x, y: predictPoly(coefficients, x) });
    }
    ...
    // Clamp the vertical window to the data so a wild high-degree curve
    // cannot squash everything into a flat line.
    ...
    return { x0, x1, y0, y1, sx, sy, curve };
  }, [all, coefficients]);
  ...
  const path = useMemo(() => {
    ...
    for (const pt of view.curve) {
      ...
      if (!inside) {
        // still draw toward the edge, then lift the pen
  ...
  const renderPoint = (p: Point, kind: "train" | "test") => {
    ...
    const yhat = predictPoly(coefficients, p.x);
      ...
      <g key={p.id}>
        ...
        {kind === "test" ? (
          <rect
            ...
            width={active ? 13 : 10}
            height={active ? 13 : 10}
            className="fill-background stroke-resid-neg cursor-pointer"
            strokeWidth={2}
            tabIndex={0}
            role="button"
  ...
  return (
    <div className="relative">
      <svg
        ...
        role="img"
        aria-label="Dataset with the fitted polynomial curve"
      >
      ...
      <p className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
        ...
        <span>Hover or click a point to inspect its prediction and residual.</span>
      </p>
    </div>
  );
}
